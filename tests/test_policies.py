...
async def test_pbac_all_actor_grants_everyone(
    ...
    # A token with a non-matching role still gets access.
...
async def test_pbac_condition_grants_when_true(
    ...
    policy = create_policy(graphapi_post, "conditional")
    declare_actor(graphapi_post, policy, "role", "conditional-role")
    declare_rule(
        graphapi_post,
        policy,
        "Query",
...
async def test_legacy_policy_bootstrapped(
    ...
    actors = graphapi_post(READ_POLICY_ACTORS, variables={"uuids": [legacy]}).data[
        "policies"
    ]["objects"][0]["actors"]
    assert {(a["kind"], a["value"]) for a in actors} == {("all", "")}

    # One explicit rule per permission-gated (type, field), each gated on the
    # field's required RBAC role. No wildcards; covers top-level queries and
    # mutators as well as nested relation fields (e.g. `Myself.policies`,
    # `Employee.addresses`).
    rules = {
        (r["type"], r["field"], r["condition"])
        for r in read_policy_rules(graphapi_post, legacy)
    }
    # No wildcard rule remains.
    assert not any(r[0] == "*" or r[1] == "*" for r in rules)
    # A representative sample of the explicit rules is present.
    assert ("Query", "employees", '"read_employee" in token.roles') in rules
    assert ("Query", "policies", '"read_policy" in token.roles') in rules
    ...
    assert ("Myself", "policies", '"read_policy" in token.roles') in rules
...
async def test_pbac_rule_filter_fails_hard_when_unevaluatable(
    ...
    async with another_transaction() as (_, session):
        await session.execute(
            ...
            .where(PolicyRuleTable.policy_fk == UUID(policy))
...
@pytest.mark.integration_test
async def test_pbac_check_spec_unknown_collection_fails_hard(
    graphapi_post: GraphAPIPost, set_settings, set_auth, empty_db, root_org
) -> None:
    # A check-spec naming a collection the engine cannot evaluate is a
    # misconfigured rule: it must fail hard (surface an error), not silently deny.
    bob = "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb"
    create_named_person(graphapi_post, bob, "Bob", "Bertelsen")
    itsystem = create_itsystem(graphapi_post)
    bob_ituser = create_ituser_for(graphapi_post, bob, itsystem, "bob-account")

    policy = create_policy(graphapi_post, "bogus-collection")
    declare_actor(graphapi_post, policy, "all", "")
    declare_rule(
        graphapi_post,
        policy,
        "Mutation",
        "ituser_update",
        filter='{"collection": "bogus", "check": "IN", "field": input.uuid, '
        '"filter": {}}',
    )
...
# Bootstrap Owner policy (end-to-end, PBAC mode, role: owner)
# -----------------------------------------------------------
...
@pytest.mark.integration_test
async def test_owner_policy_bootstrapped(graphapi_post: GraphAPIPost, empty_db) -> None:
    by_name = {p["name"]: p["uuid"] for p in read_policies(graphapi_post)}
    assert "Owner" in by_name
    owner = by_name["Owner"]

    # Bound to the "owner" role.
    actors = graphapi_post(READ_POLICY_ACTORS, variables={"uuids": [owner]}).data[
        "policies"
    ]["objects"][0]["actors"]
    assert {(a["kind"], a["value"]) for a in actors} == {("role", "owner")}

    rules = read_policy_rules(graphapi_post, owner)
    pairs = {(r["type"], r["field"]) for r in rules}
    ...
    assert all(r["condition"] is None and r["filter"] is not None for r in rules)
    # The filters reference the lazy `actor` variable.
    assert all("actor" in r["filter"] for r in rules)

    # It is a normal, removable policy (unlike the protected policyadmin one).
    deleted = graphapi_post(DELETE_POLICY, variables={"uuid": owner})
    assert deleted.errors is None
    assert owner not in {p["uuid"] for p in read_policies(graphapi_post)}
...
async def test_owner_org_unit_object_and_hierarchy(
    ...
    assert rename(owned).errors is not None
    assert rename(child).errors is not None
...
async def test_owner_employee_object_and_create(
    ...
    set_auth(role="owner", user_uuid=OWNER_Y)
    assert update_employee(person).errors is not None
    set_auth(role="owner", user_uuid=OWNER_X)
    ...
    assert created.errors is not None
...
async def test_owner_create_under_owned_parent(
    ...
    assert new_org_unit(graphapi_post, "sub2", unowned).errors is not None
    # A root unit (no parent) has no parent to own -> denied.
    assert new_org_unit(graphapi_post, "newroot", None).errors is not None
...
async def test_owner_move_requires_new_parent_ownership(
    ...
    assert move(other).errors is not None
...
async def test_owner_bulk_requires_all_items_owned(
    ...
    assert empty.errors is not None
...
async def test_owner_may_delete_owned_detail(
    ...
    set_auth(role="owner", user_uuid=OWNER_Y)
    assert graphapi_post(DELETE_ITUSER, variables={"uuid": ituser}).errors is not None
    set_auth(role="owner", user_uuid=OWNER_X)
...
async def test_owner_lazy_actor_not_resolved_when_unreferenced(
    ...
    # `actor`.
    policy = create_policy(graphapi_post, "any-ituser")
    declare_actor(graphapi_post, policy, "all", "")
    declare_rule(
        graphapi_post,
        policy,
...
async def test_pbac_rule_filter_malformed_fails_hard(
    graphapi_post: GraphAPIPost, set_settings, set_auth, empty_db, root_org, filter_cel
) -> None:
    # Every malformed/unevaluatable filter must surface an error rather than
    # silently denying.
    bob = "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb"
    create_named_person(graphapi_post, bob, "Bob", "Bertelsen")
    itsystem = create_itsystem(graphapi_post)
    ...
    policy = create_policy(graphapi_post, "malformed")
    declare_actor(graphapi_post, policy, "all", "")
    declare_rule(graphapi_post, policy, "Mutation", "ituser_update", filter=filter_cel)
