...
@pytest.mark.parametrize(
    "from_date,matches",
    [
        # By default the owner is evaluated as of now, where it has expired.
        (None, False),
        # Within the validity period the employee is matched.
        ("2020-06-01T00:00:00+01:00", True),
        # Outside the validity period the relation does not exist.
        ("2023-06-01T00:00:00+01:00", False),
    ],
)
def test_employee_owner_filter_respects_validity(
    ...
    # `from_date` gates both the ownership link (outer employee filter) and the
    # owner org-function itself (the `OwnerFilter`).
    employee_filter: dict[str, Any] = {"owner": {"owner": {"uuids": [str(owner)]}}}
    if from_date is not None:
        employee_filter["from_date"] = from_date
        employee_filter["owner"]["from_date"] = from_date
