...
def actor_filter_for(roles: Collection[str]) -> PolicyActorFilter:
    ...
    a null `roles`, which is the query API's "match any actor" existence filter.
    """
...
def validate_rule_filter(filter_value: str | None) -> None:
    ...
    Any rule may now carry a filter. The filter is a CEL expression returning one
...
async def actor_grants_field(
    ...
    role) and has a rule granting the ``(type, field)`` -- where either
    component may be the wildcard ``"*"`` -- whose CEL condition (if any) holds
    *and* whose entity filter (if any) matches the object being accessed. This
    is the core of the PBAC permission engine.
