...
def parse_filter(
    ...
    coerced = coerce_input_value(
        ...
        lambda path, invalid, error: errors.append(
            ValueError(f"{'.'.join(map(str, path))}: {error.message}")
        ),
    )
    if errors:
        # Wrapped even when alone, as a task group does, so a caller meets one shape
        raise ExceptionGroup("invalid filter", errors)
