...
def collection_denials(
    collection: Collection, keys: Sequence[AccessKey], rules: Sequence[Rule]
) -> Select[Any]:
    """Select the accesses to collection that rules do not grant."""
...
async def access_load_fn(
    ...
    """Whether the caller may read some fields of some objects, in one query at most.

    Each collection asked about selects the accesses to it to deny under the
    rules of the caller's roles, and the union of them is the query: the fields
    denied of each object, which are usually none at all. A collection the
    caller has no rule of is denied without asking, as nothing of it could be
    granted.
    """
    roles = (await get_token()).realm_access.roles
    missing: dict[tuple[Collection, UUID], set[Field]] = defaultdict(set)
    denials: list[Select[Any]] = []
    by_collection = bucket(keys, key=lambda access: access.collection)
    for collection in by_collection:
        accesses = list(by_collection[collection])
        rules = load_rules(roles, collection, accesses)
        # Without a rule nothing is granted, and there is nothing to ask about
        if not rules:
            for _, uuid, field in accesses:
                missing[(collection, uuid)].add(field)
            continue
        denials.append(collection_denials(collection, accesses, rules))
    if denials:
        ...
        for collection, uuid, fields in rows:
            missing[(collection, uuid)].update(fields)
    return [
        field not in missing.get((collection, uuid), set())
