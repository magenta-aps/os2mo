...
def build_activation(token: Token, permission: str | None = None) -> Any:
...
def build_filter_base_vars(
    token: Token, input: Any, permission: str | None = None
) -> dict:
...
async def evaluate_filter(
    ...
    ``permission``). ``loaders`` maps a lazily-resolved variable name
    (``actor``/``current``) to an async zero-argument callable producing its
    value; a loader is awaited only when the executed CEL path dereferences that
    variable (branch-sensitive), after which the expression is re-evaluated.
    Resolution is bounded by the number of lazy variables.
